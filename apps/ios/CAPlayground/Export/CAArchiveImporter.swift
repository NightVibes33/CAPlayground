...
private final class CAMLImporter: NSObject, XMLParserDelegate {
    private struct LayerDraft { var tag: String; var layer: LayerModel }
    ...
    private struct TransitionDraft { var transition: StateTransition }
    ...
    func parser(
        ...
        if beginState(elementName, attributes) { return }
        ...
        if let layerKind = kind(elementName, attributes) {
            beginLayer(elementName, layerKind, attributes)
            return
        }
        ...
        case "CGImage":
            ...
            if currentEmitterCell != nil {
                currentEmitterCell?.imageName = source.map(assetName)
        ...
        case "font":
            layerStack[index].layer.fontFamily = attributes["value"]
        case "string":
            layerStack[index].layer.text = attributes["value"] ?? ""
        case "type":
            if layerStack[index].layer.kind == .gradient {
                layerStack[index].layer.gradientType = attributes["value"] ?? attributes["type"]
            }
        case "CGColor":
            if currentAnimation != nil, animationSection == "values" {
                appendAnimationColor(attributes)
            } else if parent == "colors",
                      layerStack[index].layer.kind == .gradient,
            ...
            } else if currentEmitterCell != nil,
                      let value = attributes["value"],
                      let hex = color(value) {
                currentEmitterCell?.color = hex
                let existing = currentEmitterCell?.alpha ?? 1
                currentEmitterCell?.alpha = number(attributes["opacity"]) ?? existing
            }
        case "color":
            if currentEmitterCell != nil,
               let value = attributes["value"],
               let hex = color(value) {
                currentEmitterCell?.color = hex
                currentEmitterCell?.alpha = number(attributes["opacity"]) ?? 1
            }
        case "real", "integer":
            appendAnimationScalar(attributes)
        case "CGPoint":
            if currentAnimation != nil, animationSection == "values" {
        ...
        case "NSArray":
            if currentAnimation != nil, animationSection == "values" { animationColorArray = [] }
        case "keyTimes", "values":
            if currentAnimation != nil { animationSection = elementName }
        case "inputIntensity":
    ...
    func parser(
        ...
        if endState(elementName) { return }
    ...
    private func beginLayer(_ tag: String, _ kind: LayerKind, _ attributes: [String: String]) {
        ...
        let size = LayerSize(
            width: bounds.count >= 4 ? bounds[2] : 390,
            height: bounds.count >= 4 ? bounds[3] : 844
        )
        var layer = LayerModel(
            id: id,
            name: attributes["name"] ?? kind.title,
            kind: kind,
            position: .init(
                x: position.first ?? size.width / 2,
                y: position.count > 1 ? position[1] : size.height / 2
            ),
            size: size
        )
        ...
        if let json = attributes["caplaySyncStateFrameMode"]?.data(using: .utf8) {
            layer.syncStateFrameMode = try? JSONDecoder().decode([String: String].self, from: json)
        }
    ...
    private func beginAnimation(_ elementName: String, _ parent: String?, _ attributes: [String: String]) -> Bool {
        let valid = parent == "animations" && ["CAKeyframeAnimation", "animation", "p"].contains(elementName)
        ...
        currentAnimation = .init(
            ...
            repeats: repeats,
            repeatDuration: repeats ? nil : number(repeatDuration),
            calculationMode: attributes["calculationMode"] ?? "linear",
            timingFunction: attributes["timingFunction"] ?? "linear"
        )
        animationLayerDepth = layerStack.count
        return true
    }

    private func endAnimation(_ elementName: String) -> Bool {
        guard let draft = currentAnimation,
              ["CAKeyframeAnimation", "animation", "p"].contains(elementName),
              animationLayerDepth == layerStack.count,
              !layerStack.isEmpty else { return false }
        let keyTimes: [Double]
        if draft.keyTimes.isEmpty {
            keyTimes = draft.values.indices.map { Double($0) / Double(max(draft.values.count - 1, 1)) }
        } else {
            keyTimes = Array(draft.keyTimes.prefix(draft.values.count))
        }
        let numericValues = draft.values.compactMap { value -> Double? in
            if case .number(let number) = value { return number }
            return nil
        }
        layerStack[layerStack.count - 1].layer.animations.append(
            .init(
                enabled: true,
                keyPath: draft.keyPath,
                numericValues: numericValues,
                values: draft.values.isEmpty ? nil : draft.values,
                keyTimes: keyTimes,
                duration: draft.duration,
                autoreverses: draft.autoreverses,
                repeats: draft.repeats,
                calculationMode: draft.calculationMode,
                timingFunction: draft.timingFunction,
                repeatDurationSeconds: draft.repeatDuration,
                speed: draft.speed
            )
        )
        currentAnimation = nil
        animationLayerDepth = nil
        animationSection = nil
        animationColorArray = nil
        return true
    }

    private func appendAnimationScalar(_ attributes: [String: String]) {
        guard var animation = currentAnimation, let raw = number(attributes["value"]) else { return }
        if animationSection == "keyTimes" {
            animation.keyTimes.append(raw)
        } else if animationSection == "values" {
            let converted = animation.keyPath.hasPrefix("transform.rotation") ? raw * 180 / .pi : raw
            animation.values.append(.number(converted))
        } else { return }
    ...
    private func appendAnimationColor(_ attributes: [String: String]) {
        ...
        if animationColorArray != nil { animationColorArray?.append(stop) }
        else { currentAnimation?.values.append(.color(hex)) }
    }

    private func beginState(_ elementName: String, _ attributes: [String: String]) -> Bool {
        switch elementName {
        case "LKState":
            let name = attributes["name"] ?? "State"
            currentState = name
            if !states.contains(name), !name.lowercased().hasPrefix("base") { states.append(name) }
            return true
        case "LKStateSetValue":
            guard let target = UUID(uuidString: attributes["targetId"] ?? ""),
                  let keyPath = attributes["keyPath"] else { return true }
            currentStateSetValue = (target, keyPath)
            return true
        case "value":
            guard let state = currentState, let setValue = currentStateSetValue else { return false }
            let type = attributes["type"] ?? "real"
            ...
            else if let numeric = Double(raw) {
                value = .number(setValue.keyPath.hasPrefix("transform.rotation") ? numeric * 180 / .pi : numeric)
    ...
    private func endState(_ elementName: String) -> Bool {
        switch elementName {
        case "LKStateSetValue": currentStateSetValue = nil; return true
        case "LKState": currentState = nil; return true
        default: return false
        }
    }

    private func beginTransition(_ elementName: String, _ attributes: [String: String]) -> Bool {
        switch elementName {
        case "LKStateTransition":
            currentTransition = .init(transition: .init(
                fromState: attributes["fromState"] ?? "Locked",
                toState: attributes["toState"] ?? "Unlock",
                elements: []
            ))
            return true
        case "LKStateTransitionElement":
            guard currentTransition != nil,
                  let target = UUID(uuidString: attributes["targetId"] ?? "") else { return true }
            currentTransitionElement = .init(
                targetID: target,
                keyPath: attributes["key"] ?? attributes["keyPath"] ?? "position",
                animation: nil
            )
            return true
        case "animation":
            guard currentTransition != nil,
                  currentTransitionElement != nil,
                  elementStack.dropLast().contains("LKStateTransition") else { return false }
            currentTransitionElement?.animation = .init(
                type: attributes["type"] ?? "CASpringAnimation",
                damping: number(attributes["damping"]) ?? 10,
                mass: number(attributes["mass"]) ?? 1,
                stiffness: number(attributes["stiffness"]) ?? 100,
                initialVelocity: number(attributes["velocity"]) ?? number(attributes["initialVelocity"]) ?? 0,
                duration: number(attributes["duration"]),
                fillMode: attributes["fillMode"],
                keyPath: attributes["keyPath"],
    ...
    private func endTransition(_ elementName: String) -> Bool {
        switch elementName {
        case "LKStateTransitionElement":
            if let element = currentTransitionElement { currentTransition?.transition.elements.append(element) }
            currentTransitionElement = nil
            return true
        case "LKStateTransition":
            if let transition = currentTransition?.transition { stateTransitions.append(transition) }
    ...
    private func beginEmitterCell(_ elementName: String, _ parent: String?, _ attributes: [String: String]) -> Bool {
        guard elementName == "CAEmitterCell", parent == "emitterCells" else { return false }
        let radiansToDegrees = 180.0 / Double.pi
        currentEmitterCell = .init(
            id: UUID(uuidString: attributes["id"] ?? "") ?? UUID(),
            name: attributes["name"] ?? "Particle",
            imageName: nil,
            contentsScale: number(attributes["contentsScale"]) ?? 1,
            birthRate: number(attributes["birthRate"]) ?? 18,
            lifetime: number(attributes["lifetime"]) ?? 2.5,
            lifetimeRange: number(attributes["lifetimeRange"]) ?? 0,
            velocity: number(attributes["velocity"]) ?? 45,
            velocityRange: number(attributes["velocityRange"]) ?? 25,
            emissionLongitude: (number(attributes["emissionLongitude"]) ?? 0) * radiansToDegrees,
            emissionLatitude: (number(attributes["emissionLatitude"]) ?? 0) * radiansToDegrees,
            emissionRange: (number(attributes["emissionRange"]) ?? (2 * Double.pi)) * radiansToDegrees,
            scale: number(attributes["scale"]) ?? 0.03,
            scaleRange: number(attributes["scaleRange"]) ?? 0.02,
            scaleSpeed: number(attributes["scaleSpeed"]) ?? 0,
            alpha: number(attributes["alpha"]) ?? number(attributes["opacity"]) ?? 1,
            alphaRange: number(attributes["alphaRange"]) ?? 0,
            alphaSpeed: number(attributes["alphaSpeed"]) ?? -0.35,
            spin: (number(attributes["spin"]) ?? 0) * radiansToDegrees,
            spinRange: (number(attributes["spinRange"]) ?? 0) * radiansToDegrees,
            xAcceleration: number(attributes["xAcceleration"]) ?? 0,
            yAcceleration: number(attributes["yAcceleration"]) ?? 0,
            color: color(attributes["color"]) ?? "#FFFFFF",
            redRange: number(attributes["redRange"]) ?? 0,
            redSpeed: number(attributes["redSpeed"]) ?? 0,
            greenRange: number(attributes["greenRange"]) ?? 0,
            greenSpeed: number(attributes["greenSpeed"]) ?? 0,
            blueRange: number(attributes["blueRange"]) ?? 0,
            blueSpeed: number(attributes["blueSpeed"]) ?? 0
        )
        return true
    }

    private func endEmitterCell(_ elementName: String) -> Bool {
        guard elementName == "CAEmitterCell", let cell = currentEmitterCell else { return false }
        if !layerStack.isEmpty {
            var cells = layerStack[layerStack.count - 1].layer.emitterCells ?? []
            cells.append(cell)
            layerStack[layerStack.count - 1].layer.emitterCells = cells
        }
        currentEmitterCell = nil
        return true
    }

    private func beginFilter(_ elementName: String, _ parent: String?, _ attributes: [String: String]) -> Bool {
        guard !layerStack.isEmpty else { return false }
        let index = layerStack.count - 1
        if elementName == "compositingFilter" {
            layerStack[index].layer.blendMode = attributes["filter"] ?? attributes["name"]
            return true
        }
        ...
        currentFilterIndex = layerStack[index].layer.filters.count - 1
        return true
    }
    ...
    private func beginGyro(_ elementName: String, _ parent: String?, _ attributes: [String: String]) -> Bool {
        ...
        guard currentGyroDictionary != nil,
              ["axis", "image", "keyPath", "layerName", "mapMaxTo", "mapMinTo", "title", "view"].contains(elementName) else { return false }
        currentGyroDictionary?[elementName] = attributes["value"] ?? ""
        return true
    }

    private func endGyro(_ elementName: String) -> Bool {
        ...
        var values = layerStack[layerStack.count - 1].layer.gyroDictionaries ?? []
        values.append(.init(
            axis: dictionary["axis"] ?? "x",
            keyPath: dictionary["keyPath"] ?? "position.x",
            layerName: layerName,
    ...
    private func kind(_ name: String, _ attributes: [String: String]) -> LayerKind? {
        switch name {
    ...
    private func isLayerElement(_ name: String) -> Bool {
        ["CAGradientLayer", "CAEmitterLayer", "CATransformLayer", "CAReplicatorLayer", "CATextLayer", "CABackdropLayer", "CALayer"].contains(name)
    }

    private func applyTransform(_ attributes: [String: String], to layer: inout LayerModel) {
        if let value = number(attributes["transform.rotation.z"]) { layer.rotation = value * 180 / .pi }
        if let value = number(attributes["transform.rotation.x"]) { layer.rotationX = value * 180 / .pi }
        if let value = number(attributes["transform.rotation.y"]) { layer.rotationY = value * 180 / .pi }
        ...
        if attributes["transform.rotation.z"] == nil,
           let value = capture(#"rotate\(\s*([-+0-9.eE]+)deg"#, in: transform).flatMap(Double.init) { layer.rotation = value }
        if attributes["transform.rotation.y"] == nil,
           let value = capture(#"rotate\(\s*([-+0-9.eE]+)deg\s*,\s*0\s*,\s*1\s*,\s*0"#, in: transform).flatMap(Double.init) { layer.rotationY = value }
        if attributes["transform.rotation.x"] == nil,
           let value = capture(#"rotate\(\s*([-+0-9.eE]+)deg\s*,\s*1\s*,\s*0\s*,\s*0"#, in: transform).flatMap(Double.init) { layer.rotationX = value }
    }

    private func applyReplicatorTransform(_ attributes: [String: String], to layer: inout LayerModel) {
        if let perspective = attributes["sublayerTransform"].flatMap({ capture(#"perspective\(\s*([-+0-9.eE]+)"#, in: $0) }).flatMap(Double.init) {
        ...
        if let values = captureGroups(#"translate\(\s*([-+0-9.eE]+)\s*,\s*([-+0-9.eE]+)\s*,\s*([-+0-9.eE]+)"#, in: transform), values.count == 3 {
            layer.instanceTranslationX = Double(values[0]) ?? 0
            layer.instanceTranslationY = Double(values[1]) ?? 0
            layer.instanceTranslationZ = Double(values[2]) ?? 0
        }
        if let rotation = capture(#"rotate\(\s*([-+0-9.eE]+)deg"#, in: transform).flatMap(Double.init) {
            layer.instanceRotation = rotation
        }
    }

    private func inferVideoFrameNaming(_ source: String, layerIndex: Int) {
        let file = assetName(source)
        ...
        layerStack[layerIndex].layer.framePrefix = prefix
        layerStack[layerIndex].layer.frameExtension = ext == "." ? ".jpg" : ext
    }
    ...
    private func color(_ value: String?) -> String? {
        ...
        let r = min(max(components[0], 0), 1)
        let g = min(max(components[1], 0), 1)
        let b = min(max(components[2], 0), 1)
        return String(format: "#%02X%02X%02X", Int((r * 255).rounded()), Int((g * 255).rounded()), Int((b * 255).rounded()))
    }

    private func point(_ value: String?) -> Vector2? {
        let values = numbers(value)
        guard values.count >= 2 else { return nil }
        return .init(x: values[0], y: values[1])
    }

    private func sizeValue(_ value: String?) -> LayerSize? {
        let values = numbers(value)
        guard values.count >= 2 else { return nil }
