...
enum CAArchiveImporter {
    static func importProject(data: Data, suggestedName: String) throws -> CAProjectDocument {
        ...
        let name = suggestedName
            .replacingOccurrences(of: ".tendies", with: "", options: [.caseInsensitive])
        ...
        for entry in entries where entry.path.lowercased().contains("/assets/") {
            ...
            if assets[assetName] == nil { assets[assetName] = entry.data }
        }
...
private final class CAMLImporter: NSObject, XMLParserDelegate {
    ...
    func parser(
        ...
        case "real", "integer":
            appendAnimationScalarIfNeeded(elementName, attributes: attributes)
        case "CGPoint":
            if currentAnimation != nil, animationSection == "values" {
    ...
    func parser(
        ...
        if layerStack.isEmpty {
            ...
            layerStack[layerStack.count - 1].layer.children.append(draft.layer)
        }
    }
    ...
    private func beginAnimationElement(_ elementName: String, parent: String?, attributes: [String: String]) -> Bool {
        let isAnimationContainerItem = parent == "animations" && (
            elementName == "CAKeyframeAnimation" ||
    ...
    private func beginStateElement(_ elementName: String, attributes: [String: String]) -> Bool {
        ...
        case "value":
            ...
            if type == "string" { value = .string(raw) }
                ...
                let converted = setValue.keyPath.hasPrefix("transform.rotation") ? number * 180 / .pi : number
    ...
    private func beginTransitionElement(_ elementName: String, attributes: [String: String]) -> Bool {
        ...
        case "LKStateTransitionElement":
            ...
            currentTransitionElement = .init(targetID: target, keyPath: attributes["key"] ?? attributes["keyPath"] ?? "position", animation: nil)
            return true
        case "animation":
            guard currentTransition != nil, currentTransitionElement != nil,
    ...
    private func beginEmitterCellElement(_ elementName: String, parent: String?, attributes: [String: String]) -> Bool {
        ...
        currentEmitterCell = .init(cell: EmitterCellModel(
            ...
            emissionLongitude: number(attributes["emissionLongitude"]) ?? 0,
            emissionLatitude: number(attributes["emissionLatitude"]) ?? 0,
            emissionRange: number(attributes["emissionRange"]) ?? 360,
            ...
            spin: number(attributes["spin"]) ?? 0,
            spinRange: number(attributes["spinRange"]) ?? 0,
    ...
    private func endStyleElement(_ elementName: String) -> Bool {
        ...
        layerStack[layerStack.count - 1].layer.gyroDictionaries = gyros
        return true
    }
    ...
    private func applyTransform(_ attributes: [String: String], to layer: inout LayerModel) {
        ...
        if attributes["transform.rotation.z"] == nil,
           let degrees = capture(#"rotate\(\s*([-+0-9.eE]+)deg"#, in: transform).flatMap(Double.init) { layer.rotation = degrees }
        if attributes["transform.rotation.y"] == nil,
           let degrees = capture(#"rotate\(\s*([-+0-9.eE]+)deg\s*,\s*0\s*,\s*1\s*,\s*0"#, in: transform).flatMap(Double.init) { layer.rotationY = degrees }
        if attributes["transform.rotation.x"] == nil,
           let degrees = capture(#"rotate\(\s*([-+0-9.eE]+)deg\s*,\s*1\s*,\s*0\s*,\s*0"#, in: transform).flatMap(Double.init) { layer.rotationX = degrees }
    }
    ...
    private func assetName(_ value: String) -> String {
    ...
    private func bool(_ value: String) -> Bool {
        value == "1" || value.lowercased() == "true" || value.lowercased() == "yes"
    }
