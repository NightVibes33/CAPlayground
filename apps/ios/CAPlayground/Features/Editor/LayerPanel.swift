...
struct LayerPanel: View {
    ...
    private func layerRow(_ layer: LayerModel, depth: Int) -> some View {
        ...
        return VStack(spacing: 0) {
            ...
            .dropDestination(for: String.self) { items, point in
                ...
                let position: LayerDropPosition = protected && project.gyroEnabled ? .into : point.y < 10 ? .before : point.y > 30 ? .after : .into
    ...
    private func dropLine(_ depth: Int) -> some View { Rectangle().fill(CATheme.accent).frame(height: 2).padding(.leading, CGFloat(8 + depth * 16)) }
        ...
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") { guard let renameID else { return }; let name = renameValue.trimmingCharacters(in: .whitespacesAndNewlines); project.root.update(id: renameID) { $0.name = name }; renameOpen = false }
    ...
    private func duplicateSelected() { for id in multiSelected { _ = project.root.duplicate(id: id) } }
    private func deleteSelected() { for id in multiSelected where project.root.find(id: id).map({ !isProtected($0) }) ?? false { project.root.remove(id: id) }; if let selectedID, multiSelected.contains(selectedID) { self.selectedID = nil }; multiSelected.removeAll() }

    private func move(_ source: UUID, target: UUID?, position: LayerDropPosition) {
        ...
        if let target { project.root.insert(moving, relativeTo: target, position: position) }
        else { project.root.children.append(moving) }
        selectedID = source
    }
}
...
struct AddLayerMenu: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            if let uploadStatus { Text(uploadStatus).font(.caption).foregroundStyle(.secondary).lineLimit(1) }
            Menu {
        ...
        .fileImporter(isPresented: $imageImporterOpen, allowedContentTypes: NativeImageAssetLoader.allowedTypes, allowsMultipleSelection: true) { result in
            guard case .success(let urls) = result else { return }; Task { await importImages(urls) }
        }
        .sheet(isPresented: $videoLayerOpen) { VideoLayerDialog(project: $project, selectedID: $selectedID) }
    }

    private func add(_ kind: LayerKind) {
        let id = UUID(); let parent = selectedID.flatMap { project.root.find(id: $0) }
        let containerWidth = parent?.size.width ?? project.width, containerHeight = parent?.size.height ?? project.height
        ...
        switch kind {
        case .text: layer.text = "Text Layer"; layer.fontSize = 16; layer.textColor = "#111827"; layer.textAlignment = "center"; layer.fontFamily = "SFProText-Regular"; layer.wrapsText = true
        case .shape: layer.size = .init(width: 120, height: 120); layer.shape = "rect"; layer.fillColor = "#60A5FA"; layer.backgroundColor = "#60A5FA"
        case .gradient: layer.size = .init(width: 200, height: 200); layer.gradientType = "axial"; layer.gradientStops = [.init(color: "#FFFFFF", opacity: 1), .init(color: "#000000", opacity: 1)]; layer.gradientStart = .init(x: 0, y: 0); layer.gradientEnd = .init(x: 1, y: 1)
        case .liquidGlass: layer.size = .init(width: 200, height: 200); layer.cornerRadius = 40
        case .emitter: layer.position = .init(x: project.width / 2, y: project.height / 2); layer.size = .init(width: project.width, height: project.height); layer.emitterPosition = .init(x: 0, y: 0); layer.emitterSize = .init(width: 0, height: 0); layer.emitterShape = "point"; layer.emitterMode = "volume"; layer.renderMode = "unordered"; layer.emitterCells = []
        case .replicator: layer.size = .init(width: project.width, height: project.height); layer.instanceCount = 5; layer.instanceTranslationX = 0; layer.instanceTranslationY = 0; layer.instanceTranslationZ = 0; layer.instanceRotation = 0; layer.instanceDelay = 0
    ...
    @MainActor private func importImages(_ urls: [URL]) async {
        ...
        for url in urls {
            ...
            do {
                ...
                let id = UUID(); var layer = LayerModel(id: id, name: nextName(url.deletingPathExtension().lastPathComponent.isEmpty ? "Image Layer" : url.deletingPathExtension().lastPathComponent), kind: .image, position: insertionPosition(), size: .init(width: imageWidth * scale, height: imageHeight * scale))
                layer.imageName = filename; layer.contentMode = "fill"; insert(layer)
            } catch { uploadStatus = "Failed to import \(url.lastPathComponent)" }
        }
    }

    private func insertionPosition() -> Vector2 { let parent = selectedID.flatMap { project.root.find(id: $0) }; return .init(x: (parent?.size.width ?? project.width) / 2, y: (parent?.size.height ?? project.height) / 2) }
    private func insert(_ layer: LayerModel) { if let selectedID, project.root.find(id: selectedID)?.kind != .emitter { project.root.update(id: selectedID) { $0.children.append(layer) } } else { project.root.children.append(layer) }; selectedID = layer.id }
    private func nextName(_ base: String) -> String { let names = Set(project.root.flattened().map(\.name)); guard names.contains(base) else { return base }; var index = 2; while names.contains("\(base) \(index)") { index += 1 }; return "\(base) \(index)" }
}

@MainActor
enum NativeImageAssetLoader {
    struct Imported { let data: Data; let image: UIImage; let filename: String }
    static let svgType = UTType(filenameExtension: "svg")!
    static let allowedTypes: [UTType] = [.png, .jpeg, UTType(filenameExtension: "webp")!, UTType(filenameExtension: "bmp")!, svgType]
    ...
    static func load(_ url: URL) async throws -> Imported {
        ...
        if url.pathExtension.lowercased() == "svg" || UTType(filenameExtension: url.pathExtension)?.conforms(to: svgType) == true {
            let raster = try await rasterizeSVG(data); guard let image = UIImage(data: raster) else { throw CocoaError(.fileReadCorruptFile) }
        ...
        return .init(data: data, image: UIImage(cgImage: cgImage), filename: url.lastPathComponent.isEmpty ? "Image.png" : url.lastPathComponent)
    }
    ...
    private static func rasterizeSVG(_ data: Data) async throws -> Data {
        ...
        let web = WKWebView(frame: CGRect(x: 0, y: 0, width: 1024, height: 1024)); web.isOpaque = false; web.backgroundColor = .clear; web.scrollView.backgroundColor = .clear
        web.loadHTMLString("<html><head><meta name='viewport' content='width=device-width,initial-scale=1'></head><body style='margin:0;background:transparent;display:inline-block'>\(svg)</body></html>", baseURL: nil)
        for _ in 0..<100 { if !web.isLoading { break }; try await Task.sleep(for: .milliseconds(20)) }
        ...
        let dict = raw as? [String: Any], sourceW = (dict?["w"] as? NSNumber)?.doubleValue ?? 512, sourceH = (dict?["h"] as? NSNumber)?.doubleValue ?? 512
        let scale = min(1, 4096 / max(sourceW, sourceH, 1)), width = max(1, sourceW * scale), height = max(1, sourceH * scale)
        web.frame = CGRect(x: 0, y: 0, width: width, height: height); web.scrollView.contentSize = web.frame.size
        let configuration = WKSnapshotConfiguration(); configuration.rect = web.bounds
        let image = try await web.takeSnapshot(configuration: configuration); guard let png = image.pngData() else { throw CocoaError(.fileWriteUnknown) }; return png
    }
}
...
extension LayerModel {
    ...
    mutating func duplicate(id: UUID) -> UUID? { if let index = children.firstIndex(where: { $0.id == id }) { var copy = children[index]; copy.refreshIDs(); copy.name += " Copy"; children.insert(copy, at: index + 1); return copy.id }; for index in children.indices { if let duplicated = children[index].duplicate(id: id) { return duplicated } }; return nil }
    mutating func reorder(id: UUID, action: LayerReorderAction) { if let index = children.firstIndex(where: { $0.id == id }) { switch action { case .front: children.append(children.remove(at: index)); case .forward where index < children.count - 1: children.swapAt(index, index + 1); case .backward where index > 0: children.swapAt(index, index - 1); case .back: children.insert(children.remove(at: index), at: 0); default: break }; return }; for index in children.indices { children[index].reorder(id: id, action: action) } }
    mutating func removeReturning(id: UUID) -> LayerModel? { if let index = children.firstIndex(where: { $0.id == id }) { return children.remove(at: index) }; for index in children.indices { if let result = children[index].removeReturning(id: id) { return result } }; return nil }
    fileprivate mutating func insert(_ moving: LayerModel, relativeTo target: UUID, position: LayerDropPosition) { if let index = children.firstIndex(where: { $0.id == target }) { switch position { case .before: children.insert(moving, at: index); case .after: children.insert(moving, at: index + 1); case .into: children[index].children.append(moving) }; return }; for index in children.indices { if children[index].contains(id: target) { children[index].insert(moving, relativeTo: target, position: position); return } } }
