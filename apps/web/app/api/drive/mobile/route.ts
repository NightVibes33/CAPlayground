...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.action === "upload" && body.name && body.zipData) {
      ...
      const metadata = { name: `${body.name}.ca.zip`, parents: [folderId], description: `CAPlayground project: ${body.name}` }
      ...
      const uploaded = await driveJSON("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,webViewLink", token, { method: "POST", headers: { "Content-Type": `multipart/related; boundary=${boundary}` }, body: multipart })
      return response({ success: true, file: uploaded }, sealed)
    }
